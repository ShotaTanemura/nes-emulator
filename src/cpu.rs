...
mod test {
    ...
    #[test]
    fn test_ldx_immediate_from_memory() {
        let mut cpu = CPU::new();
        cpu.load(vec![0xA2, 0x10]);
        cpu.reset();
    ...
    #[test]
    fn test_bpl_relative_success() {
        let mut cpu = CPU::new();
        cpu.load(vec![0x10, 0x05]);
        cpu.reset();
    ...
    #[test]
    fn test_clc_implied_clear_only_carry_flag() {
        let mut cpu = CPU::new();
        cpu.load(vec![0x18]);
        cpu.reset();

        let before = cpu.program_counter;
        cpu.status = status_flag::NEGATIVE + status_flag::ZERO + status_flag::CARRY;
        cpu.run();
    ...
    #[test]
    fn test_cld_implied_clear_decimal_flag() {
        let mut cpu = CPU::new();
        cpu.load(vec![0xD8]);
        cpu.reset();

        let before = cpu.program_counter;
        cpu.status = status_flag::DECIMAL;
        cpu.run();
    ...
    #[test]
    fn test_cld_implied_clear_only_decimal_flag() {
        let mut cpu = CPU::new();
        cpu.load(vec![0xD8]);
        cpu.reset();
